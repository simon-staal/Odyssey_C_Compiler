...
void Declaration::generateMIPS(std::ostream &dst, Context &context, int destReg) const
{
  ...
  if(branches[1]->isFunction()){
    ...
    if(branches[1]->isInit()){
      ...
      context.stack.back().varBindings[id] = {size, -1*context.stack.back().offset, destReg}; // stores the space allocated
    ...
    // Variable is not initialised, space is allocated and everything is stored in context for intialisation
      ...
      context.stack.back().varBindings[id] = {size, -1*context.stack.back().offset, -1}; // stores the space allocated (currently not available in a register)
    }
  }
}
